...
(async function() {
    // --- FIX: ẨN LOGIN NGAY LẬP TỨC ĐỂ TRÁNH NHẤP NHÁY ---
    // Giúp khi đang check config (hoặc mạng chậm) sẽ không hiện Login trước
    const _overlay = document.getElementById("loginOverlay");
    if (_overlay) _overlay.style.display = "none";

    // --- PHẦN LOGIC CHECK CONFIG TỪ XA (MỚI THÊM - BẢN PRO UI) ---
    ...
    async function checkStatus() {
        try {
            ...
            if (config.status !== 'ACTION') {
                // --- GIAO DIỆN BẢO TRÌ CHUYÊN NGHIỆP (TITANIUM STYLE) ---
                document.body.innerHTML = `
                    <style>
                        ...
                        .icon-box {
                            font-size: 4rem; color: #38bdf8; margin-bottom: 25px; 
                        ...
                        .btn-retry {
                            ...
                            color: white; border-radius: 16px; font-weight: 700; cursor: pointer; transition: all 0.3s;
                            box-shadow: 0 10px 20px rgba(59, 130, 246, 0.3); 
                            text-decoration: none; display: inline-flex; align-items: center; gap: 8px; font-size: 1rem;
                            text-transform: uppercase; letter-spacing: 0.5px;
                        }
                        .btn-retry:hover { transform: translateY(-3px); box-shadow: 0 15px 35px rgba(59, 130, 246, 0.5); }
                        .link-home { margin-top: 20px; color: #64748b; font-size: 0.85rem; text-decoration: none; transition: 0.3s; }
                        .link-home:hover { color: #38bdf8; }
                    ...
                    <div class="maint-card">
                        ...
                        <p>${config.message || "Chúng tôi đang nâng cấp hệ thống để mang lại trải nghiệm tốt hơn. Vui lòng quay lại sau ít phút!"}</p>
                        <button class="btn-retry" onclick="location.reload()">
                            <i class="fa-solid fa-rotate-right"></i> Thử Lại
                        </button>
                        ${config.updateLink ? `<a href="${config.updateLink}" class="link-home" target="_blank">Trang Chủ / Hỗ Trợ</a>` : ''}
                    </div>
                `;
    ...
    function initLogin() {
        // --- FIX: KHÔI PHỤC HIỂN THỊ LOGIN (VÌ ĐÃ ẨN Ở ĐẦU SCRIPT) ---
        // Nếu là ACTION, ta trả lại thuộc tính hiển thị để CSS xử lý tiếp
